...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version")
		{
			std::clog << SOFTWARE_NAME << " Version: " << SOFTWARE_VERSION << " - Codenamed: (" << SOFTWARE_CODENAME << ")\n"
			"A server edited by: "SOFTWARE_DEVELOPERS".\n"
...
ServiceManager* services)
{
	...
	else if(defaultPriority == "higher" || defaultPriority == "normal")
	...
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Loading channels");
	#endif
	...
	if(ip.size())
	{
		...
		#if defined(WINDOWS) && !defined(__CONSOLE__)
		SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)"> Global address: ");
		#endif
		...
		if(!gethostname(hostName, 128))
		{
			if(hostent* host = gethostbyname(hostName))
			{
				...
				for(uint8_t** addr = (uint8_t**)host->h_addr_list; addr[0] != NULL; addr++)
				{
					...
					serverIps.push_front(std::make_pair(*(uint32_t*)(*addr), 0xFFFFFFFF));
	...
	std::clog << "> Total loading time: " << (OTSYS_TIME() - loadingTime) / (1000.) << " seconds." << std::endl;
}
			...
			switch(LOWORD(wParam))
			{
				...
				case ID_MENU_MAIN_CLEARLOG:
				{
					if(g_game.getGameState() != GAMESTATE_STARTUP)
					{
						...
						GUI::getInstance()->m_lineCount = 0;
						std::clog << SOFTWARE_NAME << " Version: " << SOFTWARE_VERSION << " - Codenamed: (" << SOFTWARE_CODENAME << ")" << std::endl;
						std::clog << "A server edited by: "SOFTWARE_DEVELOPERS" ." << std::endl;
						std::clog << "A server developed by Elf, Talaturen, KaczooH, Stian and Kornholijo" << std::endl;
						std::clog << "Visit our forum for updates, support and resources: http://otland.net" << std::endl;
						std::clog << "" << std::endl;
					}
				...
				{
					MessageBox(NULL, SOFTWARE_NAME" is a custom OpenTibia Server.\n"
					"The current server version is "SOFTWARE_VERSION, "About "SOFTWARE_NAME, MB_OK);
					break;
				}

				case ID_MENU_ABOUT_DEVELOPERS:
				{
					MessageBox(NULL, "The developers of "SOFTWARE_NAME" are:\n"
					SOFTWARE_DEVELOPERS".", "About Developers", MB_OK);
					break;
				}
				...
				case ID_MENU_OTSERV:
				{
					ShellExecute(NULL, "open", "http://www.otserv.es",
				...
				{
					ShellExecute(NULL, "open", "http://www.otserv.es/index.php?/topic/51-fty-server-version-20-the-second-multi-protocols",
						NULL, NULL, SW_SHOWNORMAL);
					break;
				}
...
int32_t WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszArgument, int32_t WindowStyle)
{
	...
	wincl.lpszClassName = "tfsserver_gui";
	...
	GUI::getInstance()->m_mainWindow = CreateWindowEx(0, "tfsserver_gui", SOFTWARE_NAME, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 640, 450, HWND_DESKTOP, NULL, hInstance, NULL);
