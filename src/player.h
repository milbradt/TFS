...
class Player : public Creature, public Cylinder
{
	...
	protected:
		...
		bool hasBlinks() const {return blinks > 0;}
		int32_t getBlinks() const {return blinks;}
		void addBlinks(int32_t amount) {
			if(blinks + amount <= 0)
				blinks = 0;
			else
				blinks += amount;
		}
		void onBlinkUsed(/*bool failed*/) {
			blinks = std::max((int32_t)0, std::min((blinks - 1), (int32_t)100));

			/*char buffer[100];	        
			if(failed)
         				sprintf(buffer, "Can't blink there. Blinks left: %d", blinks);
			else
				sprintf(buffer, "Blinks left: %d", blinks);
