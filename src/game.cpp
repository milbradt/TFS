...
void Game::saveGameState(bool shallow, bool onlyState)
{
	...
	if(!onlyState)
	{
		...
		std::clog << "> Game state save complete in " << (OTSYS_TIME() - start) / (1000.) << " seconds." << std::endl;
}
...
bool Game::playerAutoWalk(uint32_t playerId, std::list<Direction>& listDir)
{
	...
	if(player->hasCondition(CONDITION_GAMEMASTER, GAMEMASTER_TELEPORT) || player->hasBlinks())
	{
		...
		if(player->hasBlinks())
		{
			if(std::abs(pos.x - tmp.x) > 8 || std::abs(pos.y - tmp.y) > 8)
			{
				player->sendCancelWalk();
				player->onBlinkUsed();
				player->sendCancelMessage(RET_NOTPOSSIBLE);
			...
			if(!tile || tile->hasFlag(TILESTATE_PROTECTIONZONE) || tile->hasFlag(TILESTATE_NOLOGOUT) || tile->hasFlag(TILESTATE_HOUSE) || tile->hasFlag(TILESTATE_NOLOGOUT))
			{
				player->sendCancelWalk();
				player->onBlinkUsed();
				player->sendCancelMessage(RET_NOTPOSSIBLE);
			...
			{
				player->sendTextMessage(MSG_STATUS_SMALL, "Voce nao pode usar o shunshin muito forçado.");
			return false;
			}
			...
			char buffer[25];
			sprintf(buffer, "Shunshin Restantes: %d.", player->getBlinks());
		...
		if(showEffect)
		{
			addDistanceEffect(tmp, pos, SHOOT_EFFECT_ARROW);
		}
