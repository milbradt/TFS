...
class Spectators
{
	...
	private:
		...
		void sendCreatureSay(const Creature* creature, SpeakClasses type, const std::string& text, Position* pos)
		{
			if(!owner)
				return;

			owner->sendCreatureSay(creature, type, text, pos);
			if(type == SPEAK_PRIVATE || type == SPEAK_PRIVATE_RED || type == SPEAK_RVR_ANSWER) // care for privacy!
				return;
		...
		//void sendToChannel(const Creature* creature, SpeakClasses type, const std::string& text, uint16_t channelId); // old
		...
		/*void sendExtendedOpcode(uint8_t opcode, const std::string& buffer)
			{if(owner) owner->sendExtendedOpcode(opcode, buffer);}*/
