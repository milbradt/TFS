...
bool LuaInterface::loadDirectory(const std::string& dir, Npc* npc/* = NULL*/, bool recursively/* = false*/)
{
	...
	for(boost::filesystem::directory_iterator it(dir), end; it != end; ++it)
	{
		...
		{
			if(recursively && !loadDirectory(dir + "/" + s, npc, recursively))
				return false;
		}
...
void LuaInterface::registerFunctions()
{
	...
	//doPlayerAddBlinks(cid, amount)
	lua_register(m_luaState, "doPlayerAddBlinks", LuaInterface::luaDoPlayerAddBlinks);

	//getPlayerBlinks(cid)
	lua_register(m_luaState, "getPlayerBlinks", LuaInterface::luaGetPlayerBlinks);	
	...
	lua_register(m_luaState, "doPlayerRemoveBlessing", LuaInterface::luaDoPlayerRemoveBlessing);
...
int32_t LuaInterface::luaDoPlayerAddBlinks(lua_State* L)
{
	//doPlayerAddBlinks(cid, amount)
...
int32_t LuaInterface::luaGetPlayerBlinks(lua_State* L)
{
	//getPlayerBlinks(cid)
	ScriptEnviroment* env = getEnv();
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
		lua_pushnumber(L, player->getBlinks());
	}
	else
	{
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
...
{
	uint64_t health, corpse, distance, experience, speed;
	Outfit_t outfit;
	PositionEx pos;
	...
	pobranyTyp->nameDescription = description;
...
{
	//doSaveServer([shallow = false], [onlyState = false])
	bool shallow = false;
	if(lua_gettop(L) > 0)
		shallow = popNumber(L);
	...
	if(lua_gettop(L) > 1)
		onlyState = popNumber(L);

	Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::saveGameState, &g_game, shallow, onlyState)));
	lua_pushnil(L);
	return 1;
}
...
int32_t LuaInterface::luaDoSaveHouse(lua_State* L)
{
	...
	if(!IOMapSerialize::getInstance()->saveHouse(db, house) || !IOMapSerialize::getInstance()->saveHouseItems(db, house))
	{
		std::stringstream s;
