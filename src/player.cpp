...
{
	if(client->getOwner())
		p->setPlayer(this); //client here maybe? [fare]
	...
	lastAttack = mailAttempts = idleTime = marriage = blessings = balance = premiumDays = mana = manaMax = manaSpent = 0;
...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	if(guildId)
	{
		...
		s << (rankName.empty() ? "é membro" : rankName)<< " do Clan " << guildName;
		if(!guildNick.empty())
			s << " (" << guildNick << ")";
...
{
    uint16_t naruto, sasuke, lee, sakura, gaara, neji, kiba, shikamaru, hinata, tenten, itachi, bee, kakashi, kankuro, hidan, orochimaru, madara, chouji, obito, temari, shino, kisame, minato, pain, deidara, kabuto, sai, tsunade, raikage, hashirama, jiraya, defaultt = 0;
    ...
    bee = 2932;    // copse for bee

    kakashi = 2852;    // copse for kakashi

    kankuro  = 2815;    // copse for kankuro

    hidan    = 2881;    // copse for hidan

    orochimaru  = 2908;    // copse for orochi

    madara   = 2936;    // copse for madara

    chouji   = 2913;    // copse for choji

    obito   = 2888;    // copse for obito

    temari   = 2927;    // copse for temari   

    shino   = 2926;    // copse for shino   

    kisame   = 2920;    // copse for kisame   

    minato   = 2921;    // copse for minato   

    pain   = 2922;    // copse for pain   

    deidara   = 2880;    // copse for deidara 

    kabuto   = 2858;    // copse for kabuto 

    sai      = 2884;    // copse for sai 

    tsunade   = 2850;    // copse for tsunade 

    raikage   = 2879;    // copse for killer a 

    hashirama   = 2936;    // copse for hashirama 
    
    jiraya   = 6318;    // copse for hashirama    
        
    defaultt = 2936;    // copse for vocations not mentioned up, like if he has no vocation
    ...
    else if (getVocationId() == 30)
        return  lee;  

    else if (getVocationId() == 40)
        return sakura;  

    else if (getVocationId() == 50)
        return gaara;  

    else if (getVocationId() == 60)
        return neji;  

    else if (getVocationId() == 70)
        return kiba;  

    else if (getVocationId() == 80)
        return shikamaru;  

    else if (getVocationId() == 90) 
        return hinata;  

    else if (getVocationId() == 100)
        return tenten;

    else if (getVocationId() == 110) 
        return itachi; 

    else if (getVocationId() == 120)
        return bee;

    else if (getVocationId() == 130)
        return kakashi;

    else if (getVocationId() == 140)
        return kankuro;

    else if (getVocationId() == 150)
        return hidan;

    else if (getVocationId() == 160)
        return orochimaru; 

    else if (getVocationId() == 170)
        return madara; 

    else if (getVocationId() == 180)
        return chouji; 

    else if (getVocationId() == 190)
        return obito; 

    else if (getVocationId() == 200)
        return temari; 

    else if (getVocationId() == 210)
        return shino; 

    else if (getVocationId() == 220)
        return kisame; 

    else if (getVocationId() == 230)
        return minato; 

    else if (getVocationId() == 240)
        return pain;

    else if (getVocationId() == 250)
        return deidara; 

    else if (getVocationId() == 260)
        return kabuto;  

    else if (getVocationId() == 270)
        return sai;  

    else if (getVocationId() == 280)
        return tsunade;  

    else if (getVocationId() == 290)
        return raikage;  

    else if (getVocationId() == 300)
        return hashirama;  
        
    else if (getVocationId() == 310)
        return jiraya;          
...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_NOTMOVABLE:
			sendCancel("Nao pode mover este objeto.");
			break;
		...
		case RET_NOTENOUGHCAPACITY:
			sendCancel("Este objeto eh muito pesado para voce carregar.");
			break;

		case RET_CONTAINERNOTENOUGHROOM:
			sendCancel("Nao cabe mais objetos aqui.");
			break;
		...
		case RET_PLAYERISPZLOCKED:
			sendCancel("Voce nao pode entrar em Zona de Protecao depois de atacar alguem.");
			break;
		...
		case RET_PLAYERWITHTHISNAMEISNOTONLINE:
			sendCancel("Jogador com este nome nao esta online.");
			break;

		case RET_NOTREQUIREDLEVELTOUSERUNE:
			sendCancel("Voce nao tem ninjutsu necessario para usar este pergaminho.");
			break;

		case RET_YOUAREALREADYTRADING:
			sendCancel("Voce ja esta negociando.");
			break;

		case RET_THISPLAYERISALREADYTRADING:
			sendCancel("Este jogador ja esta negociando.");
			break;

		case RET_YOUMAYNOTLOGOUTDURINGAFIGHT:
			sendCancel("Voce nao pode sair do jogo durante seu battle ativo!");
			break;
		...
		case RET_NOTENOUGHLEVEL:
			sendCancel("Voce nao tem um level suficiente.");
			break;

		case RET_NOTENOUGHMAGICLEVEL:
			sendCancel("Voce nao tem ninjutsu suficiente.");
			break;
		...
		case RET_NOTENOUGHSOUL:
			sendCancel("Voce nao tem vigor suficiente.");
			break;
		...
		case RET_CANONLYUSETHISRUNEONCREATURES:
			sendCancel("Voce so pode usar este pergaminho em criaturas.");
			break;

		case RET_PLAYERISNOTREACHABLE:
			sendCancel("Player esta longe ainda.");
			break;

		case RET_CREATUREISNOTREACHABLE:
			sendCancel("Criatura esta longe ainda.");
			break;

		case RET_ACTIONNOTPERMITTEDINPROTECTIONZONE:
			sendCancel("Esta acao nao eh permitida em Zona de Protecao.");
			break;
		...
		case RET_YOUMAYNOTATTACKAPERSONINPROTECTIONZONE:
			sendCancel("Voce nao pode atacar um Player em uma Zona Segura.");
			break;

		case RET_YOUMAYNOTATTACKAPERSONWHILEINPROTECTIONZONE:
			sendCancel("Voce nao pode atacar um Player em quanto voce estiver em Zona Segura.");
			break;

		case RET_YOUCANONLYUSEITONCREATURES:
			sendCancel("Voce so pode usar isto em criaturas.");
			break;

		case RET_TURNSECUREMODETOATTACKUNMARKEDPLAYERS:
			sendCancel("Desligue o MODO SEGURO para atacar alguem, é uma kunai que esta na cor branca perto da sua Bota.");
			break;

		case RET_YOUNEEDPREMIUMACCOUNT:
			sendCancel("Voce precisa ser Premium Account.");
			break;

		case RET_YOUNEEDTOLEARNTHISSPELL:
			sendCancel("Voce precisa aprender este jutsu primeiro.");
			break;

		case RET_YOURVOCATIONCANNOTUSETHISSPELL:
			sendCancel("Seu personagem nao consegue usar este jutsu.");
			break;

		case RET_YOUNEEDAWEAPONTOUSETHISSPELL:
			sendCancel("Voce precisa usar um equipamento adequado para usar este jutsu.");
			break;
		...
		case RET_ACTIONNOTPERMITTEDINANOPVPZONE:
			sendCancel("Nesta zona voce nao pode executar esta acao!");
			break;
		...
		case RET_YOUNEEDAMAGICITEMTOCASTSPELL:
			sendCancel("Voce precisa de um pergaminho limpo para executar esse jutsu.");
			break;
		...
		case RET_YOUMAYNOTCASTAREAONBLACKSKULL:
			sendCancel("Voce nao pode usar jutsus de area em quanto estiver Black Akatsuki.");
			break;
...
void Player::onCreatureAppear(const Creature* creature)
{
	...
	Logger::getInstance()->eFile("/online.log", name, true);
...
void Player::drainHealth(Creature* attacker, CombatType_t combatType, int32_t damage)
{
	...
	char buffer[150];
	if(attacker)
		sprintf(buffer, "Voce perdeu %d hitpoint%s em um ataque feito por %s.", damage, (damage != 1 ? "s" : ""), attacker->getNameDescription().c_str());
	else
		sprintf(buffer, "Voce perdeu %d hitpoint%s.", damage, (damage != 1 ? "s" : ""));
...
void Player::drainMana(Creature* attacker, CombatType_t combatType, int32_t damage)
{
	...
	char buffer[150];
	if(attacker)
		sprintf(buffer, "Voce perdeu %d de chakra em um ataque feito por %s.", damage, attacker->getNameDescription().c_str());
	else
		sprintf(buffer, "Voce perdeu %d de chakra.", damage);
...
void Player::addManaSpent(uint64_t amount, bool useMultiplier/* = true*/)
{
	...
	if(currReqMana > nextReqMana || magLevel >= 140) //player has reached max magic level
		return;
	...
	while(manaSpent + amount >= nextReqMana)
	{
		...
		char advMsg[50];
		sprintf(advMsg, "Voce subiu o nivel do ninjutsu %d.", magLevel);
...
void Player::addExperience(uint64_t exp)
{
	...
	if(prevLevel != level)
	{
		...
		char advMsg[60];
		sprintf(advMsg, "Voce aumento do Level %d para o Level %d.", prevLevel, level);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);
...
void Player::removeExperience(uint64_t exp, bool updateStats/* = true*/)
{
	...
	if(prevLevel != level)
	{
		...
		char advMsg[90];
		sprintf(advMsg, "Voce regrediu do Level %d para o Level %d.", prevLevel, level);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);
	}
...
Item* Player::createCorpse(DeathList deathList)
{
	...
	std::stringstream ss;
	ss << "Este é o corpo " << getNameDescription() << ". " << (sex % 2 ? "Ele" : "Ela") << " foi morto por ";
...
{
	return std::ceil(((weapon && weapon->getAttackSpeed() != 0) ? weapon->getAttackSpeed() * (1 - (getSkill(SKILL_FIST, SKILL_LEVEL) * 0.0055)) : (vocation->getAttackSpeed() / std::max((size_t)1, getWeapons().size()) * (1 - (getSkill(SKILL_FIST, SKILL_LEVEL) * 0.0055)))));
}
